...
# はじめに
&emsp;サーバーサイドエンジニアにとってもキャッシュの仕組みの理解は重要です。

&emsp;GraphQLのクライアントには、以下のように設定を有効にするだけで効率よくキャッシュしてくれる仕組みが備わっています。
しかし、キャッシュの性能を発揮するにはスキーマ・サーバー設計を適切に行わなければなりません。
本記事では、GraphQLサーバー構築において考慮すべき点を紹介します。
※ クライアントライブラリは、Apollo ClientやRelayなどが存在が、Apollo Clientを前提とします。
```js:index.ts
...
# キャッシュの正規化
GraphQLのクエリはUserにネストされたPostsをまとめて取得するといったことができます。
...
ROOT_QUERYに保存されているオブジェクトのキーは、`フィールド名(引数)`となります。**（一応確認）**
...
仮に、サーバー側から返却されなかったとしても、クライアント側でキーを独自に設定できますが、可能な限りデフォルトの設定に従った方が良いと思います。
